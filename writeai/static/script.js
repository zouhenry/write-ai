...
function setStatusBanner(offline) {
  document.getElementById('statusBanner').style.display = offline
    ? 'flex'
    : 'none';
}
...
async function correctText() {
  ...
  try {
    ...
    suggestionsDiv.innerHTML =
      '<div class="empty-state" style="color: #e53e3e;"><p>Error checking grammar</p><small>Please try again</small></div>';
...
function displaySuggestions(suggestions) {
  ...
  if (suggestions && suggestions.length > 0) {
    const unappliedSuggestions = suggestions.filter(
      (_, index) => !appliedSuggestions.has(index),
    );

    if (unappliedSuggestions.length > 0) {
      suggestionsDiv.innerHTML = unappliedSuggestions
        .map((suggestion, originalIndex) => {
          const globalIndex = suggestions.indexOf(suggestion);

          // Use highlighted versions if available, otherwise fall back to escaped text
          const originalDisplay =
            suggestion.original_highlighted || escapeHtml(suggestion.original);
          const correctedDisplay =
            suggestion.corrected_highlighted ||
            escapeHtml(suggestion.corrected);
    ...
    suggestionsDiv.innerHTML =
      '<div class="empty-state"><p>No grammar issues found</p><small>Your text looks great!</small></div>';
    countBadge.textContent = '0';
  }
}
...
inputEl.addEventListener('keyup', () => {
  lastCaretPosition = inputEl.selectionStart;
});
...
function onSuggestionHover(e) {
  ...
  const bestSpan = findBestOccurrence(
    currentText,
    sug.original,
    isFinite(approxStart) ? approxStart : 0,
  );
  if (bestSpan) {
    ...
    const s = Math.max(
      0,
      Math.min(currentText.length, parseInt(startAttr, 10)),
    );
    const eIdx = Math.max(
      0,
      Math.min(currentText.length, parseInt(endAttr, 10)),
    );
...
async function applySingleSuggestion(suggestionIndex) {
  if (
    ...
    !currentCorrections.suggestions[suggestionIndex]
  ) {
    return;
  }
  ...
  try {
    ...
    const suggestionElement = document.querySelector(
      `.suggestion-item[data-index="${suggestionIndex}"]`,
    );
    ...
    // Show confirmation
    showToast(
      `Applied correction for ${currentCorrections.suggestions[suggestionIndex].sentence}`,
    );
  } catch (error) {
...
function clearText() {
  ...
  document.getElementById('suggestionsList').innerHTML =
    '<div class="empty-state"><p>No grammar issues found yet</p><small>Start writing and click "Check Grammar" to see suggestions</small></div>';
...
function downloadReport() {
  if (!window.jspdf || !window.jspdf.jsPDF) {
    alert(
      'PDF library failed to load. Please check your internet connection and refresh the page.',
    );
    return;
  }
  try {
    ...
    if (suggestions.length === 0) {
      ...
      suggestions.forEach((s, i) => {
        ...
        // ── Original row ──
        const origTokens = parseHighlightedTokens(
          s.original_highlighted || s.original,
          'error-word',
        );
        ...
        // ── Corrected row ──
        const corrTokens = parseHighlightedTokens(
          s.corrected_highlighted || s.corrected,
          'corrected-word',
        );
        y = renderHighlightedRow(
          doc,
    ...
    doc.text('Generated by Write AI', pageW / 2, y, { align: 'center' });
...
function saveConversations(conversations) {
  try {
    localStorage.setItem(
      'grammarLlmConversations',
      JSON.stringify(conversations),
    );
  } catch (e) {
    if (e.name === 'QuotaExceededError') {
      showToast(
        'Storage limit reached — please delete some conversations',
        true,
      );
    }
  }
...
function initConversations() {
  ...
  activeConversationId =
    localStorage.getItem('grammarLlmActiveConversationId') ||
    conversations[0].id;
...
async function rephraseText() {
  ...
  try {
    ...
    console.error('Error:', error);
    resultsDiv.innerHTML =
      '<div class="empty-state" style="color: #e53e3e;"><p>Error restructuring text</p><small>Please try again</small></div>';
  } finally {
    loading.style.display = 'none';
  }
}
...
function clearRephraseText() {
  ...
  const resultsDiv = document.getElementById('structureResults');
  resultsDiv.innerHTML =
    '<div class="empty-state"><p>No results yet</p><small>Enter text and click "Restructure" to see alternatives</small></div>';
...
async function sendChatMessage() {
  ...
  try {
    const response = await fetch('/chat', {
      ...
      body: JSON.stringify({
        message: message,
        history: chatHistory.slice(-25), // Send last 25 messages for context
      }),
    });
    ...
    if (isFirstExchange) {
      ...
      generateConversationTitle(message, data.response)
        .then((title) => {
          if (!title) return;
